...
const S = {
  ...
  bigNumber: { fontSize: 72, fontWeight: 700, textAlign: "center", lineHeight: 1.1, padding: "20px 0" },
  bigLetter: { fontSize: 96, fontWeight: 700, textAlign: "center", lineHeight: 1, padding: "10px 0" },
  ...
  stat: { display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #e0e0e0", fontSize: 14 },
  statVal: { fontWeight: 700 },
  progress: { height: 4, background: "#e0e0e0", marginBottom: 20 },
  progressFill: (pct) => ({ height: 4, background: "#000", width: `${pct}%`, transition: "width 0.2s" }),
  problem: { fontSize: 22, fontWeight: 700, textAlign: "center", lineHeight: 1.5, padding: "20px 10px", minHeight: 100 },
};
...
function genCalcProblem(difficulty = "medium") {
  ...
  if (type === "div") {
    ...
    return { display: `${b*result} ÷ ${b}`, answer: result, type: "calc" };
  }
  if (type === "decimal") {
    const ops = [
...
const problemTemplates = [
  () => { const v=randOf([60,80,90,100,110,120]),t=randOf([0.5,1,1.5,2,2.5,3]),subject=randOf(["Un train","Une voiture","Un cycliste professionnel","Un TGV"]); return { text:`${subject} ${randOf(["roule","avance","se déplace"])} à ${v} km/h pendant ${t<1?"30 minutes":t+" heure"+(t>1?"s":"")}. Quelle distance a-t-il parcourue ?`, answer:v*t, unit:"km" }; },
  () => { const dist=randOf([60,80,100,120,150,200,300]),v=randOf([50,60,80,100,120]); return { text:`Une voiture parcourt ${dist} km à ${v} km/h. Combien de temps dure le trajet ? (répondre en heures, arrondir à 2 décimales)`, answer:+(dist/v).toFixed(2), unit:"h" }; },
  () => { const qty=rand(2,8),price=rand(1,9)+0.5*rand(0,1),total=qty*price; return { text:`${qty} ${randOf(["pommes","oranges","yaourts","stylos","cahiers"])} coûtent ${total.toFixed(2)} €. Quel est le prix d'une unité ?`, answer:+price.toFixed(2), unit:"€" }; },
  () => { const w=rand(3,8),d=rand(4,12),nw=w+rand(1,4); return { text:`${w} ouvriers terminent un travail en ${d} jours. En combien de jours ${nw} ouvriers feront-ils le même travail ? (arrondir à 1 décimale)`, answer:+((w*d)/nw).toFixed(1), unit:"jours" }; },
  () => { const base=randOf([50,80,100,120,150,200,250,400,500,1000]),pct=randOf([5,10,15,20,25,30,40,50]); return { text:`Un prix de ${base} € ${randOf(["augmente","grimpe","progresse"])} de ${pct}%. Quel est le nouveau prix ?`, answer:base*(1+pct/100), unit:"€" }; },
  () => { const base=randOf([80,100,120,150,200,250,300,400,500]),pct=randOf([5,10,15,20,25,30,40,50]); return { text:`Un article coûte ${base} €. Il est soldé à −${pct}%. Quel est son prix final ?`, answer:base*(1-pct/100), unit:"€" }; },
  () => { const old=randOf([50,80,100,150,200]),newVal=old+randOf([10,15,20,25,30,40,50]); return { text:`Un salaire passe de ${old} € à ${newVal} €. Quelle est la hausse en pourcentage ? (arrondir à 1 décimale)`, answer:+((newVal-old)/old*100).toFixed(1), unit:"%" }; },
  () => { const total=randOf([24,36,48,60,80,100,120]),num=rand(1,4),den=randOf([2,3,4,5,6]),part=(total*num)/den; if(!Number.isInteger(part))return null; return { text:`${num}/${den} de ${total} kg de marchandises sont vendus. Combien de kg restent ?`, answer:total-part, unit:"kg" }; },
  () => { const km=randOf([1.5,2,2.5,3,5,7.5,10]); return { text:`Convertis ${km} km en mètres.`, answer:km*1000, unit:"m" }; },
  () => { const l=randOf([0.5,1,1.5,2,2.5,5]); return { text:`Convertis ${l} litre${l>1?"s":""} en centilitres.`, answer:l*100, unit:"cL" }; },
  () => { const v1=randOf([60,80,100]),h=rand(1,3)*10,v2=v1+randOf([10,20,30,40]); return { text:`Voiture A a ${h} km d'avance et roule à ${v1} km/h. Voiture B roule à ${v2} km/h. Dans combien d'heures B rattrape-t-elle A ? (arrondir à 2 déc.)`, answer:+(h/(v2-v1)).toFixed(2), unit:"h" }; },
  () => { const q1=rand(2,6),c1=randOf([10,20,30,40,50]),q2=rand(2,6),c2=randOf([10,20,30,40,50]); return { text:`On mélange ${q1}L d'une solution à ${c1}% avec ${q2}L à ${c2}%. Quelle est la concentration du mélange ? (arrondir à 1 déc.)`, answer:+((q1*c1+q2*c2)/(q1+q2)).toFixed(1), unit:"%" }; },
  () => { const capital=randOf([1000,2000,5000,10000]),rate=randOf([2,3,4,5,6,8,10]),years=rand(1,5); return { text:`Un capital de ${capital} € est placé à ${rate}% par an pendant ${years} an${years>1?"s":""}. Quel est l'intérêt simple total ?`, answer:capital*rate/100*years, unit:"€" }; },
];

function genProblem() { let p=null; while(!p){p=randOf(problemTemplates)();} return p; }
...
function AttentionMode({ onBack }) {
  const TOTAL_DURATION = 50, SHOW_DURATION = 3, HIDE_DURATION = 5, CYCLE = 8;
  ...
  function start() {
    setLetters(genLetterSequence(MAX_PAIRS));
    setNumbers(genNumberSequence(MAX_PAIRS));
    setLetterAnswers(Array(MAX_PAIRS).fill(null));
    setCurrentIdx(0); setElapsed(0); setVisible(true); setPhase("running");
  }
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setElapsed(e => {
        const next = e + 0.1;
        if (next >= TOTAL_DURATION) { clearInterval(timerRef.current); setVisible(false); setPhase("sum_input"); return TOTAL_DURATION; }
        setCurrentIdx(Math.floor(next / CYCLE));
        setVisible((next % CYCLE) < SHOW_DURATION);
        return next;
      });
    }, 100);
  ...
  function answerLetter(same) {
    ...
    setLetterAnswers(prev => { const copy = [...prev]; copy[currentIdx] = same; return copy; });
  }
  ...
  function submitSum() {
    ...
    for (let i = 2; i < letters.length; i++) { letterTotal++; if (letterAnswers[i] === correctLetters[i]) letterScore++; }
  ...
  if (phase === "intro") return (
    <div style={S.page}>
      ...
      <p style={{fontSize:14, lineHeight:1.7, marginBottom:20}}>
        <b>Règle :</b> Une lettre (haut) et un nombre (bas) apparaissent ensemble pendant <b>3 secondes</b>, disparaissent pendant <b>5 secondes</b>. Durée totale : <b>50 secondes</b>.<br/><br/>
        • À partir de la <b>3e lettre</b> : indique si elle est identique à celle d'<b>il y a 2 tours</b> (t−2).<br/>
        • Mémorise la <b>somme des nombres</b> — tu devras la donner à la fin.
      </p>
        ...
        {currentIdx >= 2 && (
          <div>
            ...
            <div style={{display:"flex", gap:10}}>
              <button style={{...S.btn(alreadyAnswered && letterAnswers[currentIdx]===true ? "primary":"secondary"), flex:1, marginBottom:0, opacity: canAnswer?1:0.4}} onClick={() => canAnswer && answerLetter(true)}>OUI</button>
              <button style={{...S.btn(alreadyAnswered && letterAnswers[currentIdx]===false ? "primary":"secondary"), flex:1, marginBottom:0, opacity: canAnswer?1:0.4}} onClick={() => canAnswer && answerLetter(false)}>NON</button>
            </div>
          </div>
        )}
      </div>
  ...
  if (phase === "result" && result) {
    ...
    return (
      <div style={S.page}>
        ...
        <div style={S.resultBox(letterPct === 100)}>Lettres : {result.letterScore}/{result.letterTotal} correct ({letterPct}%)</div>
        <div style={S.resultBox(sumOk)}>Somme : tu as répondu <b>{result.userSum}</b> — correct : <b>{result.correctSum}</b>{sumOk ? " ✓" : " ✗"}</div>
        <div style={S.divider} />
...
function ShellGame({ onBack }) {
  ...
  function startGame() {
    const ball = rand(0,2);
    setCurrentBall(ball); setRound(1); setScore(0); setRevealing(true); setPhase("running");
    setTimeout(() => { setRevealing(false); startShuffle(ball); }, 1500);
  }

  function startShuffle(ball) {
    setGuessed(false); setLastGuessOk(null);
    const numSwaps = 3 + rand(0,3);
    let delay = 0;
    for (let i = 0; i < numSwaps; i++) {
      const a = rand(0,2); let c = rand(0,2); while (c === a) c = rand(0,2);
      delay += 400;
      const _a=a, _c=c;
      setTimeout(() => { setCurrentBall(prev => { if(prev===_a)return _c; if(prev===_c)return _a; return prev; }); }, delay);
    }
  ...
  function guess(cupIdx) {
    ...
    setLastGuessOk(ok); if (ok) setScore(s => s+1); setGuessed(true); setRevealing(true);
    setTimeout(() => {
      setRevealing(false);
      if (round >= totalRounds) { setPhase("result"); addStat({ mode:"shell", score: score+(ok?1:0), total: totalRounds }); }
      else { const newBall=rand(0,2); setCurrentBall(newBall); setRound(r=>r+1); setRevealing(true); setTimeout(()=>{setRevealing(false);startShuffle(newBall);},1500); }
    }, 1200);
  }
  ...
  if (phase === "intro") return (
    <div style={S.page}>
      ...
      <p style={{fontSize:14, lineHeight:1.7, marginBottom:20}}>Une balle est cachée sous un verre. Les verres bougent — suis la balle avec les yeux.<br/><br/><b>{totalRounds} manches.</b></p>
  ...
  if (phase === "running") return (
    <div style={S.page}>
      <div style={{...S.stat, border:"none", marginBottom:16}}><span>Manche {round}/{totalRounds}</span><span style={S.statVal}>Score : {score}</span></div>
      <div style={{display:"flex", gap:16, marginBottom:24, justifyContent:"center"}}>
        {[0,1,2].map(i => (
          <div key={i} style={{flex:1, textAlign:"center"}}>
            <div style={{height:80, border:"3px solid #000", background: revealing&&currentBall===i?"#e0e0e0":"#fff", display:"flex", alignItems:"center", justifyContent:"center", fontSize:32, fontWeight:700, cursor:(!animating&&!guessed)?"pointer":"default", transition:"background 0.2s"}} onClick={() => guess(i)}>
              {revealing && currentBall === i ? "●" : ["A","B","C"][i]}
            </div>
      ...
      {guessed && <div style={{...S.resultBox(lastGuessOk===true), marginTop:16}}>{lastGuessOk ? "✓ Bien vu !" : "✗ Raté"}</div>}
    </div>
  );
  ...
  if (phase === "result") return (
    <div style={S.page}>
      <div style={S.sectionTitle}>Résultat</div>
      <div style={S.resultBox(score >= totalRounds*0.7)}>{score} / {totalRounds} ({Math.round(score/totalRounds*100)}%)</div>
...
function CalcMode({ onBack }) {
  ...
  function submit() {
    ...
    setStats(newStats);
    setHistory(prev => [...prev, { q:current.display||current.text, userAns:answer, correct:current.answerDisplay||String(current.answer), ok }]);
    if (mode === "count" && newStats.total >= countTarget) { addStat({ mode:"calc", subMode, correct:newStats.correct, total:countTarget, difficulty }); setPhase("result"); }
    else nextQuestion();
  }
  ...
  if (!configured) return (
    <div style={S.page}>
      ...
      <div style={S.row}>{[["calc","Calcul pur"],["problem","Problèmes"],["mixed","Mixte"]].map(([v,l]) => (<button key={v} style={S.btnSmall(subMode===v)} onClick={() => setSubMode(v)}>{l}</button>))}</div>
      <div style={S.label}>Difficulté</div>
      <div style={S.row}>{[["easy","Facile"],["medium","Moyen"],["hard","Difficile"]].map(([v,l]) => (<button key={v} style={S.btnSmall(difficulty===v)} onClick={() => setDifficulty(v)}>{l}</button>))}</div>
      <div style={S.divider} />
      ...
      {mode === "duration" && (<><div style={S.label}>Durée</div><div style={{...S.row, flexWrap:"wrap"}}>{DURATION_OPTIONS.map(d => (<button key={d} style={S.btnSmall(duration===d)} onClick={() => setDuration(d)}>{formatTime(d)}</button>))}</div></>)}
  ...
  if (phase === "running" && current) {
    ...
    return (
      <div style={S.page}>
        ...
        {current.type === "calc" ? <div style={S.problem}>{current.display}</div> : <div style={{...S.problem, fontSize:16, fontWeight:400, textAlign:"left"}}>{current.text}{current.unit && <span style={{fontSize:12, color:"#666"}}> (répondre en {current.unit})</span>}</div>}
        <input style={S.input} type="number" step="any" value={answer} onChange={e => setAnswer(e.target.value)} onKeyDown={e => { if(e.key==="Enter")submit(); }} autoFocus placeholder="?" />
        <button style={{...S.btn(), marginTop:12}} onClick={submit}>VALIDER →</button>
        {history.length > 0 && <div style={{marginTop:16, fontSize:12}}>{history.slice(-3).reverse().map((h,i) => (<div key={i} style={{...S.stat, color:h.ok?"#000":"#999"}}><span style={{flex:1, marginRight:8, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap"}}>{h.q}</span><span>{h.ok?"✓":` ✗ ${h.correct}`}</span></div>))}</div>}
      </div>
    );
  }
  ...
  if (phase === "result") return (
    <div style={S.page}>
      <div style={S.sectionTitle}>Résultat</div>
      <div style={S.resultBox(stats.correct/Math.max(1,stats.total)>=0.8)}>{stats.correct}/{stats.total} — {Math.round(stats.correct/Math.max(1,stats.total)*100)}%</div>
      ...
      <div style={{fontSize:12, marginBottom:20}}>
        ...
        {history.slice(-20).reverse().map((h,i) => (<div key={i} style={{...S.stat, color:h.ok?"#000":"#999"}}><span style={{flex:1, marginRight:8, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap"}}>{h.q}</span><span style={{whiteSpace:"nowrap"}}>{h.userAns} {h.ok?"✓":`✗ (${h.correct})`}</span></div>))}
      </div>
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{display:"flex", alignItems:"flex-end", gap:2, height:80, marginBottom:4}}>
        {dayData.map((d,i)=>(<div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center"}}><div style={{background:"#000",width:"100%",height:d.n===0?1:Math.round(d.n/maxN*70)}}/></div>))}
      </div>
      <div style={{display:"flex",gap:2}}>
        {dayData.map((d,i)=>(<div key={i} style={{flex:1,fontSize:8,textAlign:"center",color:"#666"}}>{d.label}</div>))}
      </div>
    </div>
  );
}
